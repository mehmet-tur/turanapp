...
export default function TalentDetailScreen() {
  ...
  return (
    <ScrollView contentContainerStyle={{ padding: 24, gap: 12 }}>
      <Text style={{ fontSize: 28, fontWeight: '700' }}>{talent.publicName}</Text>
      <Text>{talent.headline}</Text>
      ...
      {talent.sessionTypes.map((sessionType: any) => (
        <View key={sessionType.id} style={{ borderWidth: 1, borderRadius: 12, padding: 12 }}>
          ...
          <Text>{(sessionType.priceMinor / 100).toLocaleString('tr-TR')} {sessionType.currency}</Text>
